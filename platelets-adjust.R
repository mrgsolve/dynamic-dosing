...
mod <- param(mod, ADJUST  = 1, BMAX = 100, UNTIL = 52*168)
mod <- update(mod, outvars = outvars(mod)$capture)

data <- expand.ev(ID = 1:300, amt = 0, evid = 2)

set.seed(10203)
out <- mrgsim(mod, data = data, end  = 168*55 , delta = 2, obsonly = TRUE)
count(as.data.frame(out), epoch)

out <- mutate(out, KEEP = max(epoch)==3, .by = ID)
out <- filter(out, KEEP)
out <- filter(out, ID==nth(unique(ID), 1))

sims <- mutate(out, WEEK = time / 168)
...
p1 <- ggplot(sims, aes(WEEK, PLATELET)) + 
...
p2 <- ggplot(sims, aes(WEEK, current_dose)) + geom_line(lwd=0.6) + 
  scale_y_continuous(breaks = c(0, 100, 150 ,200), limits = c(0,210)) +
  scale_x_continuous(breaks = wk) + 
  labs(y = "Dose (mg)", x = "Time (weeks)")

p3 <- ggplot(sims, aes(WEEK, UNBOUND)) + 
...
ggsave("platelets.pdf", width = 5, height = 6, scale = 1, device = "pdf")
